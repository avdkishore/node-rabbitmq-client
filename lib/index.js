...
const consume = (params = {}, handler) => {
  ...
  const channelWrapper = connection.createChannel({
    setup(channel) {
      ...
      return Promise.all([channel.assertQueue(queueName, queueOptions), channel.prefetch(prefetch), channel.consume(queueName, data => {
        let message;
        try {
          message = JSON.parse(data.content.toString());

          return handler(message, params).then(() => {
...
const publish = (params = {}, data) => {
  ...
  const startPublishing = () => {
    /** returns a <Promise> */
    return channelWrapper.sendToQueue(queueName, data, { persistent: true }).then(() => {
...
const purgeQueue = (params = {}) => {
  ...
  return new Promise(resolve => {
    connection.createChannel({
      setup(channel) {
        // `channel` here is a regular amqplib `ConfirmChannel`.
        return Promise.all([channel.assertQueue(queueName, queueOptions), channel.purgeQueue(queueName)]);
      }
    });
